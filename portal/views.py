...
def home(request):
    ...
    feautured_sponser = Featured_Sponser.objects.values("user__username").annotate(
        count=Sum("amount")
    )
    context = {
        "user": user,
...
def faculty_post_search(request):
    ...
    requests = Finance_request.objects.filter(
        Q(title__icontains=query)
        | Q(description__icontains=query)
        | Q(achievements__icontains=query)
        | Q(other_performance__icontains=query),
        posted_by=request.user,
    )
    data["html"] = render_to_string(
        "faculty/finance_request.html",
        {
            "requests": requests,
        },
    )
...
def finance_request_chat(request, pk):
    ...
    if "interest_search_query" in request.GET:
        ...
        alumni_interests = Finance_request_Post_Response.objects.filter(
            Q(user__first_name__icontains=interest_search_query)
            | Q(user__last_name__icontains=interest_search_query),
            post=post,
        ).order_by("-id")
    else:
...
def faculty_addchat(request):
    ...
    Finance_request_Response_Message.objects.create(
        user_id=request.user.id, post_response_id=interest_id, message=message
    )
...
def finance_request_detail_page(request, pk):
    ...
    message = Finance_request_Response_Message.objects.filter(post_response__post=pk).order_by(
        "-id"
    )[:5]
    contributions = Featured_Sponser.objects.filter(
        user=request.user, student_name=request_details
    )
...
def alumini_message(request):
    ...
    Finance_request_Response_Message.objects.create(
        user=requser, post_response=interest, message=message
    )
...
def alumni_post_search(request):
    ...
    posts = Post.objects.filter(
        Q(title__icontains=query) | Q(content__icontains=query), posted_by=request.user
    )
    data["html"] = render_to_string(
        "alumni/help_desk_post.html",
        {
            "posts": posts,
        },
    )
...
def help_desk_post_chat(request, pk):
    ...
    if "interest_search_query" in request.GET:
        interest_search_query = request.GET["interest_search_query"]
        alumni_interests = PostResponse.objects.filter(
            Q(user__first_name__icontains=interest_search_query)
            | Q(user__last_name__icontains=interest_search_query),
            post=post,
        ).order_by("-id")
...
def student_post_search(request):
    data = dict()
    query = request.POST.get("query")
    posts = Tech_Help_Post.objects.filter(
        Q(title__icontains=query) | Q(content__icontains=query), posted_by=request.user
    )
    data["html"] = render_to_string(
        "student/mentor_help_post.html",
        {
            "posts": posts,
        },
    )
...
def mentor_help_post_chat(request, pk):
    ...
    if "interest_search_query" in request.GET:
        interest_search_query = request.GET["interest_search_query"]
        alumni_interests = Tech_Help_PostResponse.objects.filter(
            Q(user__first_name__icontains=interest_search_query)
            | Q(user__last_name__icontains=interest_search_query),
            post=post,
        ).order_by("-id")
...
def mentor_addchat(request):
    ...
    interest_id = request.POST.get("id")
    message = request.POST.get("message")
    Tech_Help_ResponseMessage.objects.create(
        user_id=request.user.id, post_response_id=interest_id, message=message
    )
...
def mentor_help_users_message(request):
    ...
    data["html"] = render_to_string(
        "pages/alumni_as_mentor/mentor_help_chat.html", context, request
    )
    return JsonResponse(data)
